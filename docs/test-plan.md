...
- Run: `bun run knip`
- **Pass**: Exit code 0 with no "Unused devDependencies" or "Unlisted binaries" errors. Configuration hints are acceptable.
- **Fail**: Non-zero exit code, or any devDependency / binary false positives reported.
- Run: `bun run knip` after `rm -rf node_modules && bun install`
- **Pass**: Same result as above -- clean install does not change outcome.
